...
## P0-1 RBAC 部门过滤注入（安全，阻断性） — ✅ 已修复（build_filter 部门名白名单 + 注册/上传/admin 三入口校验，test_fixlist.py）
...
## P0-2 工具 SQL 表白名单绕过（安全，阻断性） — ✅ 已修复（FROM/JOIN 子句捕获逗号分隔表名全校验，test_fixlist.py）
...
## P0-3 初始化版本控制（git init + 修正 .gitignore） — ✅ 已修复（git init + .gitignore 补 web-ui/dist 等，首次提交 33c94f0；GBK 误报：.gitignore 实为 UTF-8）
...
## P0-4 worker 消息丢失 + 任务状态撒谎（可靠性） — ✅ 已修复（PEL 启动恢复 + 失败落库 failed+error + processing 不悬挂，test_fixlist.py）
...
## P1-5 worker/connector 优雅退出 — ✅ 已修复（worker/connector 信号处理：SIGINT/SIGBREAK/SIGTERM 优雅退出）
...
## P1-6 静默吞异常修复（可观测性） — ✅ 已修复（_complete_text / clear_login_failures 加日志，降级行为不变）
...
## P1-7 async 路由内同步写盘 + 孤儿文件 — ✅ 已修复（写盘移线程池；入队失败清理文件 + DB 置 failed + 503）
...
## P1-8 Redis 连接超时统一 — ✅ 已修复（auth._redis 统一 socket_connect_timeout=5）
...
## P1-9 未声明直接依赖移入 pyproject — ✅ 已修复（uv add langgraph langchain-openai）
...
## P1-10 connector doc_id 超长导致摄入失败 — ✅ 已修复（_doc_id_for 截断 ≤64 + sha1 后缀，test_fixlist.py）
...
## P1-11 上传文件魔数校验 — ✅ 已修复（魔数校验：PDF %PDF / DOCX·PPTX PK，test_fixlist.py）
...
## P2-12 清理死依赖 — ✅ 部分（uv remove minio structlog；langchain-community 保留——ragas 0.4.3 兼容所需，删除会破坏评估链路）
...
## P2-13 解除死 pin — ⏸ 保留不动（websockets==15.0.1 是 uvicorn 兼容修复，见项目记忆；盲目放开会复发）
...
## P2-14 Prometheus 指标高基数 — ✅ 已修复（指标 label 用路由模板，404 归并 unknown，test_fixlist.py）
...
## P2-15 测试基础设施改进 — ⏸ 未处理（sleep/共享 app 实例等，低风险测试基建改进）
...
## P2-16 Alembic 数据库迁移 — ⏸ 未处理（Alembic 迁移，独立工程，建议单独排期）
...
## P2-17 lifespan 关闭钩子 — ⏸ 未处理（lifespan 关闭钩子，低风险）
...
## P2-18 tracing 全局客户端并发初始化 — ✅ 已修复（tracing._get_client 双检锁）
...
## P2-19 日志中用户问题脱敏 — ✅ 已修复（_complete_json 日志片段过脱敏引擎）
...
## P2-20 ruff 存量 35 个错误 — ✅ 已修复（ruff 0 错误；E501 折行 + tests 目录 per-file-ignores；ASYNC240 评估：queue 移 to_thread，connector 单任务标注）
...
## P2-21 CI/CD 流水线 — ⏸ 待 git remote 后补 CI（git 已 init；评估 job 依赖真实 LLM key 需手动触发）
...
## P2-22 应用容器化 — ⏸ 未处理（应用容器化，较大工程，建议单独排期）
